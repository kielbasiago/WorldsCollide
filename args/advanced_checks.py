...
force_reward_titles = (character_title, esper_item_title, esper_title, item_title)

def options(args):
    opts = []

    if args.check_preset:
        preset_checks = args.character_rewards or args.esper_item_rewards or args.esper_rewards or args.item_rewards
        opts.append((preset_title, preset_checks))
    else:
        opts.append((preset_title, []))
        if args.character_rewards:
            opts.append((character_title, args.character_rewards))
        if args.esper_item_rewards:
            opts.append((esper_item_title, args.esper_item_rewards))
        if args.esper_rewards:
            opts.append((esper_title, args.esper_rewards))
        if args.item_rewards:
            opts.append((item_title, args.item_rewards))

    if args.dragon_reward == RewardType.NONE:
        opts.append(('Dragon Rewards', 'None'))
    elif args.dragon_reward & RewardType.CHARACTER:
        opts.append(('Dragon Rewards', "C+E+I"))
    elif args.dragon_reward & RewardType.ESPER:
        opts.append(('Dragon Rewards', "E+I"))
    else:
        opts.append(('Dragon Rewards', "Item"))
...
def menu(args):
    from menus.submenu_force_item_reward_checks import (
        FlagsCheckPreset,
        FlagsCharacterRewardChecks,
        FlagsEsperItemRewardChecks,
        FlagsEsperRewardChecks,
        FlagsItemRewardChecks,
    )
    ...
    for index, entry in enumerate(entries):
        ...
        elif key == character_title:
            entries[index] = (character_title, FlagsCharacterRewardChecks(character_title, value)) # flags sub-menu
        elif key == esper_item_title:
            entries[index] = (esper_item_title, FlagsEsperItemRewardChecks(esper_item_title, value)) # flags sub-menu
        elif key == esper_title:
            entries[index] = (esper_title, FlagsEsperRewardChecks(esper_title, value)) # flags sub-menu
        elif key == item_title:
            entries[index] = (item_title, FlagsItemRewardChecks(item_title, value)) # flags sub-menu
...
def log(args):
    ...
    for entry in entries:
        key, value = entry
        if key == preset_title or key in force_reward_titles:
            if len(value) == 0:
                entry = (key, value)
            else:
